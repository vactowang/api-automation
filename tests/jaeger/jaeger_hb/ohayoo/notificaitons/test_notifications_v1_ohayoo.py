...
class TestNotificationsV1Ohayoo(object):
    ...
    @allure.description('Verify that the default ordinal view count in nurl for Ohayoo')
    @allure.severity('smoke')
    @pytest.mark.parametrize('pub_app_id', [common_test_app])
    def test_default_ordinal_view_ount_in_nurl_ohayoo(self, pub_app_id):
    ...
    @allure.tag('smoke', 'R_v0.31.0')
    @allure.story('PBJ-2140 New Bidding Endpoint for Ohayoo')
    @allure.description('Verify that the default ordinal view count in lurl for Ohayoo')
    @allure.severity('smoke')
    @pytest.mark.parametrize('pub_app_id', [common_test_app])
    def test_default_ordinal_view_ount_in_lurl_ohayoo(self, pub_app_id):
        test_ifa = gen_device_id()
        req = request_payload.hbp_ohayoo(pub_app_id, common_test_placement, ifa=test_ifa, bid_token=super_token)
        r = post_hbp_request(hbp_ohayoo_endpoint_qa, json=req, headers=hbp_headers(openrtb='2.5'))
