...
class TestNotificationsV1TopOn(object):
    ...
    @allure.description('Verify that the default ordinal view count in nurl for topon')
    @allure.severity('smoke')
    @pytest.mark.parametrize('pub_app_id', [common_test_app])
    def test_default_ordinal_view_ount_in_nurl_topon(self, pub_app_id):
    ...
    @allure.tag('smoke')
    @allure.story('PBJ-2784 HBP new endpoint for Fyber & TopOn')
    @allure.description('Verify that the default ordinal view count in lurl for topon')
    @allure.severity('smoke')
    @pytest.mark.parametrize('pub_app_id', [common_test_app])
    def test_default_ordinal_view_ount_in_lurl_topon(self, pub_app_id):
        test_ifa = gen_device_id()
        req = request_payload.hbp_topon(pub_app_id, common_test_placement, ifa=test_ifa, bid_token=super_token)
        r = post_hbp_request(hbp_topon_endpoint_qa, json=req, headers=hbp_headers(openrtb='2.5'))
    ...
    @allure.description('Verify that the default ordinal view count in burl for topon')
    @allure.severity('smoke')
    @pytest.mark.parametrize('pub_app_id', [common_test_app])
    def test_default_ordinal_view_ount_in_burl_topon(self, pub_app_id):
        test_ifa = gen_device_id()
        req = request_payload.hbp_topon(pub_app_id, common_test_placement, ifa=test_ifa, bid_token=super_token)
        r = post_hbp_request(hbp_topon_endpoint_qa, json=req, headers=hbp_headers(openrtb='2.5'))
