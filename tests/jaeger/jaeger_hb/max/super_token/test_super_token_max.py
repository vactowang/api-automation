...
@allure.epic('HBP Max')
class TestSuperTokenMax(object):

    @pytest.fixture(scope="class", autouse=True)
    def get_jaeger_response_bid_token_max(self):
    ...
    def test_hbp_with_valid_super_token_max(self, pub_app_id):
    ...
    @allure.severity('smoke')
    @pytest.mark.parametrize('pub_app_id', [common_test_app])
    def test_hbp_with_invalid_super_token_max(self, pub_app_id):
        test_ifa = gen_device_id(36)
        req = request_payload.hbp_max(pub_app_id, common_test_placement, ifa=test_ifa, bid_token='1:123456')
        r = post_hbp_request(hbp_max_endpoint_qa, json=req, headers=hbp_headers(openrtb='2.5'))
    ...
    @allure.severity('smoke')
    @pytest.mark.parametrize('pub_app_id', [common_test_app])
    def test_hbp_with_non_matched_super_token_max(self, pub_app_id):
        test_ifa = gen_device_id(36)
    ...
    @allure.severity('smoke')
    @pytest.mark.parametrize('pub_app_id', [common_test_app])
    def test_hbp_with_invalid_placement_non_matched_super_token_max(self, pub_app_id):
        test_ifa = gen_device_id(36)
        req = request_payload.hbp_max(pub_app_id, 'ABCDEFG', ifa=test_ifa, bid_token=super_token)
        r = post_hbp_request(hbp_max_endpoint_qa, json=req, headers=hbp_headers(openrtb='2.5'))
