...
class TestNotificationsV1Saygames(object):
    ...
    @allure.description('Verify that the default ordinal view count in nurl for Saygames')
    @allure.severity('smoke')
    @pytest.mark.parametrize('pub_app_id', [common_test_app])
    def test_default_ordinal_view_ount_in_nurl_saygames(self, pub_app_id):
    ...
    @allure.tag('smoke', 'R_v0.33.0')
    @allure.story('PBJ-2177 new endpoint for saygames')
    @allure.description('Verify that the default ordinal view count in lurl for Saygames')
    @allure.severity('smoke')
    @pytest.mark.parametrize('pub_app_id', [common_test_app])
    def test_default_ordinal_view_ount_in_lurl_saygames(self, pub_app_id):
        test_ifa = gen_device_id()
        req = request_payload.hbp_saygames(pub_app_id, common_test_placement, ifa=test_ifa, bid_token=super_token)
        r = post_hbp_request(hbp_saygames_endpoint_qa, json=req, headers=hbp_headers(openrtb='2.5'))
